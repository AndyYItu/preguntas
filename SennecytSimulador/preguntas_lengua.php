...
if (!isset($_SESSION['tiempo_limite_lengua']) || time() > $_SESSION['tiempo_limite_lengua']) {
    $_SESSION['tiempo_limite_lengua'] = time() + $tiempoLimite;
}

$materia = 'lengua';

// Verifica si el usuario ya vio la nota final
if (isset($_SESSION['nota_vista_lengua']) && $_SESSION['nota_vista_lengua']) {
    mostrarNota($_SESSION['nota_lengua']);
    exit();
}
...
if (time() > $_SESSION['tiempo_limite_lengua'] || todosContestados($_SESSION['respuestas'][$materia])) {
    $nota = calcularNota($_SESSION['preguntas_seleccionadas'][$materia], $_SESSION['respuestas'][$materia]);
    $_SESSION['nota_lengua'] = $nota;
    $_SESSION['nota_vista_lengua'] = true;
    mostrarNota($nota);
    exit();
}
...
if (isset($_POST['evaluar'])) {
    ...
    header("Location: preguntas_lengua.php?pregunta=$preguntaIndex");
    exit();
}
...
<head>
    ...
    <script>
        var tiempoRestante = <?php echo $_SESSION['tiempo_limite_lengua'] - time(); ?>;
        function actualizarTemporizador() {
            if (tiempoRestante > 0) {
                ...
                window.location.href = 'preguntas_lengua.php?terminar_intento=1';
            }
        }
...
<div class="bienvenido">
    <h2>Bienvenido al Test de Lengua</h2>
    <img src="imagenes/imageneslengua.png" alt="Descripción de la imagen" style="width: 5%; height: auto;">
</div>
...
<div class="simulador-container">
    ...
    if (!empty($_SESSION['preguntas_seleccionadas'][$materia])) {
        ...
        foreach ($_SESSION['preguntas_seleccionadas'][$materia] as $key => $pregunta) {
            ...
            echo "<a href='preguntas_lengua.php?pregunta=$key' class='cuadro-pregunta$claseContestada'>" . ($key + 1) . "</a>";
        }
