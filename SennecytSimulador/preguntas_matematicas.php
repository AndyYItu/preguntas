...
if (!isset($_SESSION['tiempo_limite_matematicas']) || time() > $_SESSION['tiempo_limite_matematicas']) {
    $_SESSION['tiempo_limite_matematicas'] = time() + $tiempoLimite;
}

$materia = 'matematicas';

// Verifica si el usuario ya vio la nota final
if (isset($_SESSION['nota_vista_matematicas']) && $_SESSION['nota_vista_matematicas']) {
    mostrarNota($_SESSION['nota_matematicas']);
    exit();
}
...
if (time() > $_SESSION['tiempo_limite_matematicas'] || todosContestados($_SESSION['respuestas'][$materia])) {
    $nota = calcularNota($_SESSION['preguntas_seleccionadas'][$materia], $_SESSION['respuestas'][$materia]);
    $_SESSION['nota_matematicas'] = $nota;
    $_SESSION['nota_vista_matematicas'] = true;
    mostrarNota($nota);
    exit();
}

if (isset($_POST['evaluar'])) {
    $preguntaIndex = isset($_POST['pregunta_index']) ? intval($_POST['pregunta_index']) : 0;
    ...
    header("Location: preguntas_matematicas.php?pregunta=$preguntaIndex");
    exit();
}
...
<head>
    <meta charset="UTF-8">
    <title>Preguntas de Matemáticas</title>
    <link rel="stylesheet" href="preguntas.css">

    <script>
        var tiempoRestante = <?php echo $_SESSION['tiempo_limite_matematicas'] - time(); ?>;
        function actualizarTemporizador() {
            if (tiempoRestante > 0) {
                ...
                window.location.href = 'preguntas_matematicas.php?terminar_intento=1';
            }
        }
...
<div class="bienvenido">
    <h2>Bienvenido al Test de Matemáticas</h2>
    <img src="imagenes/imagenesmatematicas.png" alt="Descripción de la imagen" style="width: 5%; height: auto;">
</div>
...
<div class="simulador-container">
    ...
    if (!empty($_SESSION['preguntas_seleccionadas'][$materia])) {
        ...
        echo "<input type='hidden' name='pregunta_index' value='$preguntaIndex'>";
        ...
        foreach ($_SESSION['preguntas_seleccionadas'][$materia] as $key => $pregunta) {
            ...
            echo "<a href='preguntas_matematicas.php?pregunta=$key' class='cuadro-pregunta$claseContestada'>" . ($key + 1) . "</a>";
        }
        echo "<form method='post' action=''>";
