...
if (!isset($_SESSION['tiempo_limite_historia']) || time() > $_SESSION['tiempo_limite_historia']) {
    $_SESSION['tiempo_limite_historia'] = time() + $tiempoLimite;
}

$materia = 'historia';

// Verifica si el usuario ya vio la nota final
if (isset($_SESSION['nota_vista_historia']) && $_SESSION['nota_vista_historia']) {
    mostrarNota($_SESSION['nota_historia']);
    exit();
}
...
if (time() > $_SESSION['tiempo_limite_historia'] || todosContestados($_SESSION['respuestas'][$materia])) {
    $nota = calcularNota($_SESSION['preguntas_seleccionadas'][$materia], $_SESSION['respuestas'][$materia]);
    $_SESSION['nota_historia'] = $nota;
    $_SESSION['nota_vista_historia'] = true;
    mostrarNota($nota);
    exit();
}
...
if (isset($_POST['evaluar'])) {
    ...
    header("Location: preguntas_historia.php?pregunta=$preguntaIndex");
    exit();
}
...
<head>
    <meta charset="UTF-8">
    <title>Preguntas de Historia</title>
    <link rel="stylesheet" href="preguntas.css">

    <script>
        var tiempoRestante = <?php echo $_SESSION['tiempo_limite_historia'] - time(); ?>;
        function actualizarTemporizador() {
            if (tiempoRestante > 0) {
                ...
                window.location.href = 'preguntas_historia.php?terminar_intento=1';
            }
        }
...
<div class="bienvenido">
    <h2>Bienvenido al Test de Historia</h2>
    <img src="imagenes/imageneshistoria.png" alt="Descripción de la imagen" style="width: 5%; height: auto;">
</div>
...
<div class="simulador-container">
    ...
    if (!empty($_SESSION['preguntas_seleccionadas'][$materia])) {
        ...
        foreach ($_SESSION['preguntas_seleccionadas'][$materia] as $key => $pregunta) {
            ...
            echo "<a href='preguntas_historia.php?pregunta=$key' class='cuadro-pregunta$claseContestada'>" . ($key + 1) . "</a>";
        }
