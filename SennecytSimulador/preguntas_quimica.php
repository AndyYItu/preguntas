...
if (!isset($_SESSION['tiempo_limite_quimica']) || time() > $_SESSION['tiempo_limite_quimica']) {
    $_SESSION['tiempo_limite_quimica'] = time() + $tiempoLimite;
}

$materia = 'quimica';

// Verifica si el usuario ya vio la nota final
if (isset($_SESSION['nota_vista_quimica']) && $_SESSION['nota_vista_quimica']) {
    mostrarNota($_SESSION['nota_quimica']);
    exit();
}
...
if (time() > $_SESSION['tiempo_limite_quimica'] || todosContestados($_SESSION['respuestas'][$materia])) {
    $nota = calcularNota($_SESSION['preguntas_seleccionadas'][$materia], $_SESSION['respuestas'][$materia]);
    $_SESSION['nota_quimica'] = $nota;
    $_SESSION['nota_vista_quimica'] = true;
    mostrarNota($nota);
    exit();
}
...
if (isset($_POST['evaluar'])) {
    ...
    header("Location: preguntas_quimica.php?pregunta=$preguntaIndex");
    exit();
}
...
<head>
    <meta charset="UTF-8">
    <title>Preguntas de Química</title>
    <link rel="stylesheet" href="preguntas.css">

    <script>
        var tiempoRestante = <?php echo $_SESSION['tiempo_limite_quimica'] - time(); ?>;
        function actualizarTemporizador() {
            if (tiempoRestante > 0) {
                ...
                window.location.href = 'preguntas_quimica.php?terminar_intento=1';
            }
        }
...
<div class="bienvenido">
    <h2>Bienvenido al Test de Química</h2>
    <img src="imagenes/imagenesquimica.png" alt="Descripción de la imagen" style="width: 5%; height: auto;">
</div>
...
<div class="simulador-container">
    ...
    if (!empty($_SESSION['preguntas_seleccionadas'][$materia])) {
        ...
        foreach ($_SESSION['preguntas_seleccionadas'][$materia] as $key => $pregunta) {
            ...
            echo "<a href='preguntas_quimica.php?pregunta=$key' class='cuadro-pregunta$claseContestada'>" . ($key + 1) . "</a>";
        }
        echo "<form method='post' action=''>";
