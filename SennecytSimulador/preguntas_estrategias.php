...
if (!isset($_SESSION['tiempo_limite_estrategias']) || time() > $_SESSION['tiempo_limite_estrategias']) {
    $_SESSION['tiempo_limite_estrategias'] = time() + $tiempoLimite;
}

$materia = 'estrategias';

// Verifica si el usuario ya vio la nota final
if (isset($_SESSION['nota_vista_estrategias']) && $_SESSION['nota_vista_estrategias']) {
    mostrarNota($_SESSION['nota_estrategias']);
    exit();
}
...
if (time() > $_SESSION['tiempo_limite_estrategias'] || todosContestados($_SESSION['respuestas'][$materia])) {
    $nota = calcularNota($_SESSION['preguntas_seleccionadas'][$materia], $_SESSION['respuestas'][$materia]);
    $_SESSION['nota_estrategias'] = $nota;
    $_SESSION['nota_vista_estrategias'] = true;
    mostrarNota($nota);
    exit();
}
...
if (isset($_POST['evaluar'])) {
    ...
    if (isset($_POST['respuesta_' . $preguntaIndex])) {
        ...
        if ($preguntaIndex < count($_SESSION['preguntas_seleccionadas'][$materia])) {
            header("Location: preguntas_estrategias.php?pregunta=$preguntaIndex");
            exit();
        } else {
            header("Location: preguntas_estrategias.php?terminar_intento=1");
            exit();
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Preguntas de Estrategias</title>
    <link rel="stylesheet" href="preguntas.css">

    <script>
        var tiempoRestante = <?php echo $_SESSION['tiempo_limite_estrategias'] - time(); ?>;
        function actualizarTemporizador() {
            if (tiempoRestante > 0) {
                ...
                window.location.href = 'preguntas_estrategias.php?terminar_intento=1';
            }
        }
...
<div class="bienvenido">
    <h2>Bienvenido al Test de Estrategias</h2>
    <img src="imagenes/imagenesestrategias.png" alt="Descripción de la imagen" style="width: 5%; height: auto;">
</div>
...
<div class="simulador-container">
    ...
    if (!empty($_SESSION['preguntas_seleccionadas'][$materia])) {
        ...
        foreach ($_SESSION['preguntas_seleccionadas'][$materia] as $key => $pregunta) {
            ...
            echo "<a href='preguntas_estrategias.php?pregunta=$key' class='cuadro-pregunta$claseContestada'>" . ($key + 1) . "</a>";
        }
